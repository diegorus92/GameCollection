...
namespace GameCollectionAPI_BL.Services
{
    public class GameService : IGameService
    {
        ...
        //Add new game and bind it to an user
        public string AddGame(GameDTO game, long userId)
        {
            //Looking for User
            User? user = _databaseContext.Users.FirstOrDefault(u => u.UserId == userId);
            if (user == null) return $"User -<{userId}>- doesn't exist";
            ...
            else                      //Image added
            {
                ...
                else
                {
                    try
                    {
                        ...
                        gameToSave.GameImage = game.ImageFile.FileName;
                    }
                    catch(Exception ex)
                    {
            ...
            return $"Game: -<{gameToSave.GameName}>- saved successfully for user -<{user.UserId}>-";
        }




        //Add new game without bind it to an user
        public string AddGame(GameDTO game)
        {
            //Check if The game already exist
            Game gameToSave = _databaseContext.Games.FirstOrDefault(g => g.GameName == game.GameName);
            if (gameToSave != null) return $"this game -<{game.GameName}>- already exist";
            ...
            _databaseContext.SaveChanges();
            _databaseContext.Dispose();
            return $"Game: -<{gameToSave.GameName}>- saved successfully";
        }
        ...
        public async Task<IEnumerable<GameResponseDTO>> GetGames()
        {
            ...
            foreach (Game game in games)
            {
                gamesResponse.Add(new GameResponseDTO
                {
                    ...
                    GameImages = game.GameImages.Select(images => images.GameImageName).ToList(),
